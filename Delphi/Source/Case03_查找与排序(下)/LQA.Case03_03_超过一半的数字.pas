...
//解法1：排序后返回arr[N/2], NLg(N)
procedure Solution1(arr: TArr_int);
begin
...
// 解法2：hash统计
procedure Solution2(arr: TArr_int);
begin
  // 用 hash 表计行统计
end;

// 解法3：顺序统计,O(N)，限制：需要改动数组的内容
procedure Solution3(arr: TArr_int);
...
// 不同的数，进行消除，O(N)
procedure Solution4(arr: TArr_int);
var
...
procedure Solution5(arr: TArr_int);
var
