...
uses
  Classes,
  SysUtils,DeepStar.DSA.Interfaces,DeepStar.DSA.Tree.BstNode,
...
type
  ...
  private type
    ...
    TImpl_V = TImpl<V>;
    TPtr_V = TPtr_V<V>;
    TArr_TBSNode = TArray<TBSTNode_K_V>;
  ...
  private
    ...
    procedure __reBalance(node: TBSTNode_K_V); overload;
    procedure __reBalance(pqs: TArr_TBSNode); overload;
...
function TAVLtree<K, V>.IsBalance: boolean;
begin
  Result := not __unBalance(_root);
end;

procedure TAVLtree<K, V>.Remove(key: K);
var
...
function TAVLtree<K, V>.__firstUnbalance(n: TBSTNode_K_V): TArr_TBSNode;
var
  s, p, g: TBSTNode_K_V;
  res: TArr_TBSNode;
begin
...
procedure TAVLtree<K, V>.__leftRotate(p, q: TBSTNode_K_V);
var
  pIsLeft: boolean;
  pp, B: TBSTNode_K_V;
begin
...
procedure TAVLtree<K, V>.__reBalance(pqs: TArr_TBSNode);
var
  p, q, s: TBSTNode_K_V;
begin
  if (pqs = nil) then
...
procedure TAVLtree<K, V>.__reBalance(node: TBSTNode_K_V);
var
...
procedure TAVLtree<K, V>.__rightRotate(p, q: TBSTNode_K_V);
var
  pIsLeft: boolean;
  pp, x: TBSTNode_K_V;
begin
...
function TAVLtree<K, V>.__unBalance(g: TBSTNode_K_V): boolean;
var
  minus: integer;
begin
  if g = nil then
